...
const AllLedgerApis = {
    ...
    getledgerlistinglayertwo: (req, res, next) => {

        const body = req.body;
        const account_head_name = body.account_head_name ?? null;

        if (account_head_name.length == 0 || account_head_name == null) {
            res?.send({ Status: 400, Count: 0, Message: 'Enter Valid Account Head Name!', Data: [] });
            next();
            return;
        }
    ...
    party.party_name AS account_head_name,
	(Select sum((CASE 
        WHEN from_party_id = party.id THEN -ABS(voucher_amount_dollar) 
        ELSE voucher_amount_dollar 
     END)) from erp_voucher where to_party_id=party.id) as credit_amount_dollar,
     (Select sum((CASE 
        WHEN from_party_id = party.id THEN -ABS(voucher_amount_dirham) 
        ELSE voucher_amount_dirham 
     END)) from erp_voucher where to_party_id=party.id) as credit_amount_dirham,
     (Select sum((CASE 
        WHEN from_party_id = party.id THEN -ABS(voucher_amount_dollar) 
        ELSE voucher_amount_dollar 
...
FROM 
    ...
    getledgerlistinglayerthird: (req, res, next) => {
...
FROM 
    ...
    getledgerlistingdata: (req, res, next) => {
        ...
        // var getaccountdata = `SELECT account_head_id,SUM(party_opening_balance) as amount,opening_balance_type FROM partys WHERE id='${party_id}' AND is_delete_status='0'`;
        var getaccountdata = `SELECT account_type as account_head_id, 0 as amount, NULL as opening_balance_type FROM partys WHERE id='${party_id}'`;
        conn.query(getaccountdata, (error, accountheaddata) => {
            ...
            if (accountHeadId == 27) {
                ...
                UNION
                SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.from_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.voucher_type IN('receipt','payment') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.to_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.voucher_type IN('journal') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
            }
            if ([26, 77, 30, 76, 73, 78].includes(accountHeadId)) {
                ledgerQuery[0] = `SELECT mill_invoice.id as ref_page_id,'milltaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'mill invoice' as particular,mill_invoice.mill_tax_invoice_no as invoice_no, DATE_FORMAT(mill_invoice.mill_tax_invoice_date, '%d-%m-%Y') as ledger_date, -ABS(mill_invoice.mill_tax_invoice_total_amount) as amount, mill_invoice.mill_tax_invoice_entry_date as entry_date FROM erp_mill_tax_invoice as mill_invoice WHERE mill_invoice.mill_id='${party_id}' AND mill_invoice.user_id='${user_id}' AND mill_invoice.year_id<='${year_id}' AND mill_invoice.company_id<='${company_id}' AND mill_invoice.branch_id<='${branch_id}' AND mill_invoice.is_delete_status='0' AND mill_invoice.mill_tax_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT job_invoice.id as ref_page_id,'jobtaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'jober invoice' as particular,job_invoice.jober_invoice_no as invoice_no, DATE_FORMAT(job_invoice.jober_invoice_date, '%d-%m-%Y') as ledger_date, -ABS(job_invoice.jober_invoice_total_total) as amount, job_invoice.jober_invoice_entry_date as entry_date FROM erp_jober_invoice as job_invoice WHERE job_invoice.jober_id='${party_id}' AND job_invoice.user_id='${user_id}' AND job_invoice.year_id<='${year_id}' AND job_invoice.company_id<='${company_id}' AND job_invoice.branch_id<='${branch_id}' AND job_invoice.is_delete_status='0' AND job_invoice.jober_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT pur_invoice.id as ref_page_id,'purchasetaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'purchase invoice' as particular,pur_invoice.purchase_tax_invoice_no as invoice_no, DATE_FORMAT(pur_invoice.purchase_tax_invoice_date, '%d-%m-%Y') as ledger_date, -ABS(pur_invoice.purchase_tax_invoice_total_amount) as amount, pur_invoice.purchase_tax_invoice_entry_date as entry_date FROM erp_purchase_tax_invoice as pur_invoice WHERE pur_invoice.supplier_id='${party_id}' AND pur_invoice.user_id='${user_id}' AND pur_invoice.year_id<='${year_id}' AND pur_invoice.company_id<='${company_id}' AND pur_invoice.branch_id<='${branch_id}' AND pur_invoice.is_delete_status='0' AND pur_invoice.purchase_tax_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT pur_journal.id as ref_page_id,'journalpurchase' as ref_page,'' as ref_type,0 as ref_type_id,'journal purchase' as particular,pur_journal.journal_purchase_invoice_no as invoice_no, DATE_FORMAT(pur_journal.journal_purchase_invoice_date, '%d-%m-%Y') as ledger_date, -ABS(pur_journal.journal_purchase_total_amount) as amount, pur_journal.journal_purchase_entry_date as entry_date FROM erp_journal_purchase as pur_journal WHERE pur_journal.supplier_id='${party_id}' AND pur_journal.user_id='${user_id}' AND pur_journal.year_id<='${year_id}' AND pur_journal.company_id<='${company_id}' AND pur_journal.branch_id<='${branch_id}' AND pur_journal.is_delete_status='0' AND pur_journal.journal_purchase_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT pur_return.id as ref_page_id,'purchasereturn' as ref_page,'' as ref_type,0 as ref_type_id,'purchase return' as particular,pur_return.purchase_return_note_no as invoice_no, DATE_FORMAT(pur_return.purchase_return_date, '%d-%m-%Y') as ledger_date, pur_return.purchase_return_amount as amount, pur_return.purchase_return_entry_date as entry_date FROM erp_purchase_return as pur_return WHERE pur_return.supplier_id='${party_id}' AND pur_return.user_id='${user_id}' AND pur_return.year_id<='${year_id}' AND pur_return.company_id<='${company_id}' AND pur_return.branch_id<='${branch_id}' AND pur_return.is_delete_status='0' AND pur_return.purchase_return_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.to_party_id='${party_id}' THEN voucher.voucher_amount ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.voucher_type IN('receipt','payment') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.from_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.voucher_type IN('journal') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
            }
            if ([32, 33].includes(accountHeadId)) {
                // ledgerQuery[0] = `SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.to_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
                ledgerQuery[0] = `SELECT '' as ref_page_id,'' as ref_page,voucher.voucher_type as ref_type,'' as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,'' as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.to_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
            }
            if ([59, 60, 61, 62, 63, 64].includes(accountHeadId)) {
                let gstQuery = [''];
                if ([59, 60, 61].includes(accountHeadId)) {
                    let saleTaxColName = `sale_tax_invoice_total_${accountHeadId == 59 ? 'sgst' : (accountHeadId == 60 ? 'cgst' : 'igst')}`
                    let saleReturnColName = `sale_return_total_${accountHeadId == 59 ? 'sgst' : (accountHeadId == 60 ? 'cgst' : 'igst')}`

                    gstQuery[0] = `SELECT sale_invoice.id as ref_page_id,'saletaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'sale invoice' as particular,sale_invoice.sale_tax_invoice_no as invoice_no, DATE_FORMAT(sale_invoice.sale_tax_invoice_date, '%d-%m-%Y') as ledger_date, -ABS(sale_invoice.${saleTaxColName}) as amount, sale_invoice.sale_tax_invoice_entry_date as entry_date FROM erp_sale_tax_invoice as sale_invoice WHERE sale_invoice.${saleTaxColName}>0 AND sale_invoice.user_id='${user_id}' AND sale_invoice.year_id<='${year_id}' AND sale_invoice.company_id<='${company_id}' AND sale_invoice.branch_id<='${branch_id}' AND sale_invoice.is_delete_status='0' AND sale_invoice.sale_tax_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    SELECT sale_return.id as ref_page_id,'salereturn' as ref_page,'' as ref_type,0 as ref_type_id,'sale return' as particular,sale_return.sale_return_note_no as invoice_no, DATE_FORMAT(sale_return.sale_return_date, '%d-%m-%Y') as ledger_date, sale_return.${saleReturnColName} as amount, sale_return.sale_return_entry_date as entry_date FROM erp_sale_return as sale_return WHERE sale_return.${saleReturnColName}>0 AND sale_return.user_id='${user_id}' AND sale_return.year_id<='${year_id}' AND sale_return.company_id<='${company_id}' AND sale_return.branch_id<='${branch_id}' AND sale_return.is_delete_status='0' AND sale_return.sale_return_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION `;
                }

                if ([62, 63, 64].includes(accountHeadId)) {
                    let purTaxColName = `purchase_tax_invoice_total_${accountHeadId == 62 ? 'sgst' : (accountHeadId == 63 ? 'cgst' : 'igst')}`
                    let purReturnColName = `purchase_return_total_${accountHeadId == 62 ? 'sgst' : (accountHeadId == 63 ? 'cgst' : 'igst')}`
                    let millTaxColName = `mill_tax_invoice_total_${accountHeadId == 62 ? 'sgst' : (accountHeadId == 63 ? 'cgst' : 'igst')}`
                    let purjournalTaxColName = `journal_purchase_child_total_${accountHeadId == 62 ? 'sgst' : (accountHeadId == 63 ? 'cgst' : 'igst')}`
                    let jobTaxColName = `jober_invoice_total_${accountHeadId == 62 ? 'sgst' : (accountHeadId == 63 ? 'cgst' : 'igst')}`

                    gstQuery[0] = `SELECT mill_invoice.id as ref_page_id,'milltaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'mill invoice' as particular,mill_invoice.mill_tax_invoice_no as invoice_no, DATE_FORMAT(mill_invoice.mill_tax_invoice_date, '%d-%m-%Y') as ledger_date, mill_invoice.${millTaxColName} as amount, mill_invoice.mill_tax_invoice_entry_date as entry_date FROM erp_mill_tax_invoice as mill_invoice WHERE mill_invoice.${millTaxColName}>0 AND mill_invoice.user_id='${user_id}' AND mill_invoice.year_id<='${year_id}' AND mill_invoice.company_id<='${company_id}' AND mill_invoice.branch_id<='${branch_id}' AND mill_invoice.is_delete_status='0' AND mill_invoice.mill_tax_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    SELECT job_invoice.id as ref_page_id,'jobtaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'jober invoice' as particular,job_invoice.jober_invoice_no as invoice_no, DATE_FORMAT(job_invoice.jober_invoice_date, '%d-%m-%Y') as ledger_date, job_invoice.${jobTaxColName} as amount, job_invoice.jober_invoice_entry_date as entry_date FROM erp_jober_invoice as job_invoice WHERE job_invoice.${jobTaxColName}>0 AND job_invoice.user_id='${user_id}' AND job_invoice.year_id<='${year_id}' AND job_invoice.company_id<='${company_id}' AND job_invoice.branch_id<='${branch_id}' AND job_invoice.is_delete_status='0' AND job_invoice.jober_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    SELECT pur_invoice.id as ref_page_id,'purchasetaxinvoice' as ref_page,'' as ref_type,0 as ref_type_id,'purchase invoice' as particular,pur_invoice.purchase_tax_invoice_no as invoice_no, DATE_FORMAT(pur_invoice.purchase_tax_invoice_date, '%d-%m-%Y') as ledger_date, pur_invoice.${purTaxColName} as amount, pur_invoice.purchase_tax_invoice_entry_date as entry_date FROM erp_purchase_tax_invoice as pur_invoice WHERE pur_invoice.${purTaxColName}>0 AND pur_invoice.user_id='${user_id}' AND pur_invoice.year_id<='${year_id}' AND pur_invoice.company_id<='${company_id}' AND pur_invoice.branch_id<='${branch_id}' AND pur_invoice.is_delete_status='0' AND pur_invoice.purchase_tax_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    SELECT pur_journal.id as ref_page_id,'journalpurchase' as ref_page,'' as ref_type,0 as ref_type_id,'journal purchase' as particular,pur_journal.journal_purchase_invoice_no as invoice_no, DATE_FORMAT(pur_journal.journal_purchase_invoice_date, '%d-%m-%Y') as ledger_date, pur_journal.${purjournalTaxColName} as amount, pur_journal.journal_purchase_entry_date as entry_date FROM erp_journal_purchase as pur_journal WHERE pur_journal.${purjournalTaxColName}>0 AND pur_journal.user_id='${user_id}' AND pur_journal.year_id<='${year_id}' AND pur_journal.company_id<='${company_id}' AND pur_journal.branch_id<='${branch_id}' AND pur_journal.is_delete_status='0' AND pur_journal.journal_purchase_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    SELECT pur_return.id as ref_page_id,'purchasereturn' as ref_page,'' as ref_type,0 as ref_type_id,'purchase return' as particular,pur_return.purchase_return_note_no as invoice_no, DATE_FORMAT(pur_return.purchase_return_date, '%d-%m-%Y') as ledger_date, -ABS(pur_return.${purReturnColName}) as amount, pur_return.purchase_return_entry_date as entry_date FROM erp_purchase_return as pur_return WHERE pur_return.${purReturnColName}>0 AND pur_return.user_id='${user_id}' AND pur_return.year_id<='${year_id}' AND pur_return.company_id<='${company_id}' AND pur_return.branch_id<='${branch_id}' AND pur_return.is_delete_status='0' AND pur_return.purchase_return_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION `;
                }

                ledgerQuery[0] = `${gstQuery[0]} SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.to_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
            }
            if ([6, 7].includes(accountHeadId)) {
                ledgerQuery[0] = `
                SELECT pur_journal.id as ref_page_id,'journalpurchasechild' as ref_page,'' as ref_type,0 as ref_type_id,'' as particular,'' as invoice_no, DATE_FORMAT(pur_journal.journal_purchase_invoice_date, '%d-%m-%Y') as ledger_date, pur_child_journal.journal_purchase_child_taxable as amount, pur_child_journal.journal_purchase_child_entry_date as entry_date FROM erp_journal_purchase as pur_journal INNER JOIN erp_journal_purchase_child as pur_child_journal ON pur_child_journal.journal_purchase_id=pur_journal.id WHERE pur_child_journal.supplier_id='${party_id}' AND pur_journal.user_id='${user_id}' AND pur_journal.year_id<='${year_id}' AND pur_journal.company_id<='${company_id}' AND pur_journal.branch_id<='${branch_id}' AND pur_child_journal.is_delete_status='0' AND pur_journal.is_delete_status='0' AND pur_journal.journal_purchase_invoice_date BETWEEN '${from_date}' AND '${to_date}'
                UNION
                SELECT pur_return.id as ref_page_id,'purchasereturnchild' as ref_page,'' as ref_type,0 as ref_type_id,'' as particular,pur_return.purchase_return_note_no as invoice_no, DATE_FORMAT(pur_return.purchase_return_date, '%d-%m-%Y') as ledger_date, -ABS(pur_child_return.purchase_return_child_taxable) as amount, pur_return.purchase_return_entry_date as entry_date FROM erp_purchase_return as pur_return INNER JOIN erp_purchase_return_child as pur_child_return ON pur_child_return.purchase_return_id=pur_return.id WHERE pur_child_return.party_id='${party_id}' AND pur_return.user_id='${user_id}' AND pur_return.year_id<='${year_id}' AND pur_return.company_id<='${company_id}' AND pur_return.branch_id<='${branch_id}' AND pur_return.is_delete_status='0' AND pur_child_return.is_delete_status='0' AND pur_return.purchase_return_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
            }
            if ([31, 75, 18, 16].includes(accountHeadId)) {

                // Ravi Comment
                // ledgerQuery[0] = `SELECT voucher.id as ref_page_id,'voucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM partys as party WHERE CASE WHEN voucher.from_party_id='${party_id}' THEN party.id=voucher.to_party_id ELSE party.id=voucher.from_party_id END) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,(CASE WHEN voucher.from_party_id='${party_id}' THEN -ABS(voucher.voucher_amount) ELSE voucher.voucher_amount END) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE (voucher.from_party_id='${party_id}' OR voucher.to_party_id='${party_id}') AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
                // console.log("ledgerQuery[0]::",ledgerQuery[0]);

            }

            ledgerQuery[0] = `SELECT 
    voucher.id AS ref_page_id, 
    'voucher' as ref_page,
    voucher.voucher_type AS ref_type,
    0 as ref_type_id,

    (SELECT 
        party.party_name                         
     FROM partys as party 
     WHERE 
         CASE 
             WHEN voucher.from_party_id='${party_id}' THEN party.id = voucher.to_party_id
             ELSE party.id = voucher.from_party_id 
         END
    ) as particular,     
 	voucher.voucher_no as invoice_no,
    DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,
    (CASE 
        WHEN voucher.from_party_id = '${party_id}' THEN -ABS(voucher.voucher_amount_dollar) 
        ELSE voucher.voucher_amount_dollar 
     END) AS amount_dollar,
     (CASE 
        WHEN voucher.from_party_id = '${party_id}' THEN -ABS(voucher.voucher_amount_dirham) 
        ELSE voucher.voucher_amount_dirham 
     END) AS amount_dirham,
    voucher.voucher_entry_date AS entry_date
FROM 
    erp_voucher AS voucher
WHERE 
    voucher.from_party_id = '${party_id}' 
    OR voucher.to_party_id = '${party_id}'
ORDER BY 
    entry_date ASC;`

            console.log(ledgerQuery[0]);


            if (ledgerQuery[0]?.length <= 0) {
                console.log("ledgerQuery::", ledgerQuery);
                return res?.send({ Status: 400, Count: 0, Message: 'Query Not Found!!!!', Data: error });
            }
            conn.query(ledgerQuery[0], (error, data) => {
                if (error || data?.length == 0)
                    return res?.send({ Status: 400, Count: 0, Message: 'Data Not Found!!!!', Data: error });
                else {
                    let ledgerData = [...openingBalanceData, ...data];
                    return res?.send({ Status: 200, Count: ledgerData?.length, Message: 'Data found', Data: ledgerData });
                }
            });
            return false;
    ...
    getledgerchartdata: (req, res, next) => {
        ...
        setTimeout(() => {
            res?.send({
                Status: 200, Count: 1, Message: 'Data found', Data: {
                    label: label,
                    data: chartData
                }
            });
