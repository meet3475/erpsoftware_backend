    ...
    getledgerlistingdata: (req,res,next) => {
        ...
        conn.query(getaccountdata,(error,accountheaddata) => {
            if (accountheaddata[0]?.account_head_id == '59' || accountheaddata[0]?.account_head_id == '60' || accountheaddata[0]?.account_head_id == '61' || accountheaddata[0]?.account_head_id == '62' || accountheaddata[0]?.account_head_id == '63' || accountheaddata[0]?.account_head_id == '64' || accountheaddata[0]?.account_head_id == '56' || accountheaddata[0]?.account_head_id == '54' || accountheaddata[0]?.account_head_id == '89' || accountheaddata[0]?.account_head_id == '90' || accountheaddata[0].account_head_id == '27' || accountheaddata[0].account_head_id == '26' || accountheaddata[0].account_head_id == '40' || accountheaddata[0].account_head_id == '16') 
            {
                ...
                if (accountheaddata[0].account_head_id == '27') 
                {
                    ...
                    UNION
                    SELECT voucher.id as ref_page_id,'receiptvoucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM erp_party as party WHERE party.id=voucher.to_party_id) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,-ABS(voucher.voucher_amount) as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE voucher.from_party_id='${party_id}' AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}'
                    UNION
                    ...
                    UNION
                    SELECT voucher.id as ref_page_id,'paymentvoucher' as ref_page,voucher.voucher_type as ref_type,0 as ref_type_id,(SELECT party.party_name FROM erp_party as party WHERE party.id=voucher.from_party_id) as particular,voucher.voucher_no as invoice_no, DATE_FORMAT(voucher.voucher_date, '%d-%m-%Y') as ledger_date,voucher.voucher_amount as amount, voucher.voucher_entry_date as entry_date FROM erp_voucher as voucher WHERE voucher.to_party_id='${party_id}' AND voucher.user_id='${user_id}' AND voucher.year_id<='${year_id}' AND voucher.company_id<='${company_id}' AND voucher.branch_id<='${branch_id}' AND voucher.is_delete_status='0' AND voucher.voucher_date BETWEEN '${from_date}' AND '${to_date}' ORDER BY entry_date ASC;`;
                    ...
                    //         return;
                    //     }
                    //     console.log('ledgerdata[0] :: ', ledgerdata[0])
                    //     ledgerdata[0]?.forEach(ledgervalue => {
                    //         ledgervalue.ledger_date = constant.moment(ledgervalue.ledger_date).format('YYYY-MM-DD');
                    //         // ledgervalue.amount = ledgervalue.amount == '' ? 0 : ledgervalue.amount;
                    //     });
                            
                    //     res?.send({ Status:200,Count:ledgerdata[0]?.length,Message:'Data sssss found',Data:ledgerdata[0] });
                    //     next();
                    //     return;
                    // });
                }
