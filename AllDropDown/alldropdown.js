...
const AllDropDownApis = {
    ...
    allpartybrokertransporterdropdown: (req, res, next) => {
        ...
        const new_type_id = body?.type_id?.length > 0 ?
            `AND party.party = "bank" ${(body?.type_id[0] == "bank") ? `AND party.account_type = '16'` : (body?.type_id[0] == "cash") ? `AND party.account_type = '18'` : ''}` : "";
        if (user_id == 0) {
            res?.send({ Status: 400, Count: 0, Message: 'Enter User ID', Data: [] });
            next();
            return;
