...
const AllDropDownApis = {
            ...
            {   
                ...
                {   
                    ...
                    data[0]?.forEach((typedata,index) => {
                        ...
                        const tdsratedata = `SELECT rate.id as tds_rate_id,rate.status_id,rate.tds_rate as rate,tds_status.status_name as label,rate.status_id as value FROM erp_status as tds_status LEFT JOIN erp_tds_rate as rate ON rate.status_id=tds_status.id WHERE rate.nature_of_payment_id='${typedata.value}' AND rate.user_id='${user_id}' AND rate.company_id<='${company_id}' AND rate.branch_id<='${branch_id}' AND rate.year_id<='${year_id}' GROUP BY rate.status_id ORDER BY rate.status_id ASC`;
    ...
    lastlfnumber: (req,res,next) => {
        ...
        if (invoice_type == 'greyinvoice' || invoice_type == 'finishinvoice') 
        {
            ...
            const greyinvoicelfnumber = `SELECT COALESCE(purchase_tax_invoice_no,0) as lf_no FROM erp_purchase_tax_invoice WHERE quality_type_id = '${quality_type_id}' AND user_id='${user_id}' AND company_id='${company_id}' AND branch_id='${branch_id}' AND year_id='${year_id}' AND is_delete_status='0' ORDER BY id DESC LIMIT 1 `;
        ...
        {
            const salejoberinvoicenumber = `SELECT COALESCE(jober_invoice_no,0) as jober_invoice_no,COALESCE(jober_invoice_lf_no,0) as jober_invoice_lf_no FROM erp_jober_invoice WHERE user_id='${user_id}' AND company_id='${company_id}' AND branch_id='${branch_id}' AND year_id='${year_id}' AND is_delete_status='0' ORDER BY jober_invoice_no DESC LIMIT 1 `;
