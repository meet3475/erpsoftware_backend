                                ...
                                "childdata":[
                                    {
                                        ...
                                        "amount":balancesheetphase1portdata[9].map(e => e.amount).reduce((a, b) => a + b),
                                        "childdata":balancesheetphase1portdata[9]
                                    },
                                    {
                                        "particular":"CGST",
                                        "debit_total":Cgst_Total,
                                        "amount":balancesheetphase1portdata[10].map(e => e.amount).reduce((a, b) => a + b),
                                    ...
                                    },
                                    {
                                        ...
                                        "amount":balancesheetphase1portdata[12].map(e => e.amount).reduce((a, b) => a + b),
        ...
        conn.query(balancesheetphase2portdata,[user_id,company_id,branch_id,year_id,from_date],(error,balancesheetphase2portdata) => {
            ...
            {                   
                ...
                balancesheetphase2portdata[0]?.forEach(interim_cgst_payable => {
                    Interim_Cgst_Payable_Total = Interim_Cgst_Payable_Total + Number(interim_cgst_payable.amount);
                });

                let Interim_Igst_Payable_Total = 0;
                balancesheetphase2portdata[1].forEach(interim_igst_payable => {
                    Interim_Igst_Payable_Total = Interim_Igst_Payable_Total + Number(interim_igst_payable.amount);
                });

                let Interim_Sgst_Payable_Total = 0;
                balancesheetphase2portdata[2].forEach(interim_sgst_payable => {
                    Interim_Sgst_Payable_Total = Interim_Sgst_Payable_Total + Number(interim_sgst_payable.amount);
                });

                let Sundry_Creditors_Total = 0;
                balancesheetphase2portdata[3].forEach(sundry_creditors => {
                    Sundry_Creditors_Total = Sundry_Creditors_Total + Number(sundry_creditors.amount);
                });

                let Mill_Total = 0;
                balancesheetphase2portdata[4].forEach(mill => {
                    Mill_Total = Mill_Total + Number(mill.amount);
                });

                let Creditors_Total = 0;
                balancesheetphase2portdata[5].forEach(creditors => {
                    Creditors_Total = Creditors_Total + Number(creditors.amount);
                });

                let Grey_Total = 0;
                balancesheetphase2portdata[6].forEach(grey => {
                    Grey_Total = Grey_Total + Number(grey.amount);
                });

                let Job_Work_Total = 0;
                balancesheetphase2portdata[7].forEach(job_work => {
                    Job_Work_Total = Job_Work_Total + Number(job_work.amount);
                });

                let Tds_Total = 0;
                balancesheetphase2portdata[8].forEach(tds => {
                    Tds_Total = Tds_Total + Number(tds.amount);
                });
                                ...
                                "childdata":[
                                    {
                                        ...
                                        "amount":balancesheetphase2portdata[5].map(e => e.amount).reduce((a, b) => a + b),
                                        "childdata":balancesheetphase2portdata[5]
                                    },
                                    {
                                        "particular":"Grey",
                                        "debit_total":Grey_Total,
                                        "amount":balancesheetphase2portdata[6].map(e => e.amount).reduce((a, b) => a + b),
                                        "childdata":balancesheetphase2portdata[6]
                                    },
                                    {
                                        "particular":"Job Work",
                                        "debit_total":Job_Work_Total,
                                        "amount":balancesheetphase2portdata[7].map(e => e.amount).reduce((a, b) => a + b),
