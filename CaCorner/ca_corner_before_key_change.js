...
const AllCaCornerApis = {
    ...
    balancesheetphase1portdata: (req,res,next) => {
        ...
        conn.query(balancesheetphase1portdata,[user_id,company_id,branch_id,year_id,from_date],(error,balancesheetphase1portdata) => {
            ...
            {                   
                const Capital_Account_total = (Number(balancesheetphase1portdata[0][0]?.opening) + (balancesheetphase1portdata[0][0]?.debit)) - (balancesheetphase1portdata[0][0]?.credit);

                const Reserves_Surplus_Total = (Number(balancesheetphase1portdata[1][0]?.opening) + balancesheetphase1portdata[1][0]?.debit) - (balancesheetphase1portdata[1][0]?.credit);

                const Profit_Loss_Total = (Number(balancesheetphase1portdata[1][0]?.opening) + balancesheetphase1portdata[1][0]?.debit) - (balancesheetphase1portdata[1][0]?.credit);

                let Bank_Od_Ac_Total = 0;   
                balancesheetphase1portdata[2].forEach(bank_od_ac => {
                    Bank_Od_Ac_Total = Bank_Od_Ac_Total + (Number(bank_od_ac.opening) + bank_od_ac.debit) - (bank_od_ac.credit);
                });

                let Secured_Loans_Total = 0;
                balancesheetphase1portdata[3].forEach(secured_loans => {
                    Secured_Loans_Total = Secured_Loans_Total + (Number(secured_loans.opening) + secured_loans.debit) - (secured_loans.credit);
                });

                let Unsecured_Loans_Total = 0;
                balancesheetphase1portdata[4].forEach(unsecured_loans => {
                    Unsecured_Loans_Total = Unsecured_Loans_Total + (Number(unsecured_loans.opening) + unsecured_loans.debit) - (unsecured_loans.credit);
                });

                let Broker_Total = 0;
                balancesheetphase1portdata[5].forEach(broker => {
                    Broker_Total = Broker_Total + (Number(broker.opening) + broker.debit) - (broker.credit);
                });

                let Cgst_Payable_Total = 0;
                balancesheetphase1portdata[6].forEach(cgst_payable => {
                    Cgst_Payable_Total = Cgst_Payable_Total + (Number(cgst_payable.opening) + cgst_payable.debit) - (cgst_payable.credit);
                });

                let Creditor_Other_Total = 0;
                balancesheetphase1portdata[7].forEach(creditor_other => {
                    Creditor_Other_Total = Creditor_Other_Total + (Number(creditor_other.opening) + creditor_other.debit) - (creditor_other.credit);
                });
                
                let Transporter_Total = 0;
                balancesheetphase1portdata[8].forEach(transporter => {
                    Transporter_Total = Transporter_Total + (Number(transporter.opening) + transporter.debit) - (transporter.credit);
                });

                let Sgst_Total = 0;
                balancesheetphase1portdata[9].forEach(sgst => {
                    Sgst_Total = Sgst_Total + (Number(sgst.opening) + sgst.debit) - (sgst.credit);
                });

                let Cgst_Total = 0;
                balancesheetphase1portdata[10].forEach(cgst => {
                    Cgst_Total = Cgst_Total + (Number(cgst.opening) + cgst.debit) - (cgst.credit);
                });

                let Igst_Total = 0;
                balancesheetphase1portdata[11].forEach(igst => {
                    Igst_Total = Igst_Total + (Number(igst.opening) + igst.debit) - (igst.credit);
                });

                let Tcs_Receivable_Total = 0;
                balancesheetphase1portdata[12].forEach(igst => {
                    Tcs_Receivable_Total = Tcs_Receivable_Total + (Number(igst.opening) + igst.debit) - (igst.credit);
                });

                let Gst_Payable_Total = 0;
                balancesheetphase1portdata[13].forEach(gst_payable => {
                    Gst_Payable_Total = Gst_Payable_Total + (Number(gst_payable.opening) + gst_payable.debit) - (gst_payable.credit);
                });

                let Igst_Payable_Total = 0;
                balancesheetphase1portdata[14].forEach(igst_payable => {
                    Igst_Payable_Total = Igst_Payable_Total + (Number(igst_payable.opening) + igst_payable.debit) - (igst_payable.credit);
                });

                let Sgst_Payable_Total = 0;
                balancesheetphase1portdata[15].forEach(sgst_payable => {
                    Sgst_Payable_Total = Sgst_Payable_Total + (Number(sgst_payable.opening) + sgst_payable.debit) - (sgst_payable.credit);
                });
                ...
                const allbalancesheetphase1data = [
                    {
                        ...
                        "type":"Left",
                        "childdata":balancesheetphase1portdata[1]
                    },
                    {
    ...
    balancesheetphase2portdata: (req,res,next) => {
        ...
        conn.query(balancesheetphase2portdata,[user_id,company_id,branch_id,year_id,from_date],(error,balancesheetphase2portdata) => {
            ...
            {                   
                let Interim_Cgst_Payable_Total = 0;
                balancesheetphase2portdata[0]?.forEach(interim_cgst_payable => {
                    Interim_Cgst_Payable_Total = Interim_Cgst_Payable_Total + (Number(interim_cgst_payable.opening) + interim_cgst_payable.debit) - (interim_cgst_payable.credit);
                });

                let Interim_Igst_Payable_Total = 0;
                balancesheetphase2portdata[1].forEach(interim_igst_payable => {
                    Interim_Igst_Payable_Total = Interim_Igst_Payable_Total + (Number(interim_igst_payable.opening) + interim_igst_payable.debit) - (interim_igst_payable.credit);
                });

                let Interim_Sgst_Payable_Total = 0;
                balancesheetphase2portdata[2].forEach(interim_sgst_payable => {
                    Interim_Sgst_Payable_Total = Interim_Sgst_Payable_Total + (Number(interim_sgst_payable.opening) + interim_sgst_payable.debit) - (interim_sgst_payable.credit);
                });

                let Sundry_Creditors_Total = 0;
                balancesheetphase2portdata[3].forEach(sundry_creditors => {
                    Sundry_Creditors_Total = Sundry_Creditors_Total + (Number(sundry_creditors.opening) + sundry_creditors.debit) - (sundry_creditors.credit);
                });

                let Mill_Total = 0;
                balancesheetphase2portdata[4].forEach(mill => {
                    Mill_Total = Mill_Total + (Number(mill.opening) + mill.debit) - (mill.credit);
                });

                let Creditors_Total = 0;
                balancesheetphase2portdata[5].forEach(creditors => {
                    Creditors_Total = Creditors_Total + (Number(creditors.opening) + creditors.debit) - (creditors.credit);
                });

                let Grey_Total = 0;
                balancesheetphase2portdata[6].forEach(grey => {
                    Grey_Total = Grey_Total + (Number(grey.opening) + grey.debit) - (grey.credit);
                });

                let Job_Work_Total = 0;
                balancesheetphase2portdata[7].forEach(job_work => {
                    Job_Work_Total = Job_Work_Total + (Number(job_work.opening) + job_work.debit) - (job_work.credit);
                });

                let Tds_Total = 0;
                balancesheetphase2portdata[8].forEach(tds => {
                    Tds_Total = Tds_Total + (Number(tds.opening) + tds.debit) - (tds.credit);
                });
    ...
    balancesheetphase3portdata: (req,res,next) => {
        ...
        conn.query(balancesheetphase3portdata,[user_id,company_id,branch_id,year_id,from_date],(error,balancesheetphase3portdata) => {
            ...
            {                   
                let Fixed_Assets_Total = 0;   
                balancesheetphase3portdata[0]?.forEach(fixed_assets => {
                    Fixed_Assets_Total = Fixed_Assets_Total + (Number(fixed_assets.opening) + fixed_assets.debit) - (fixed_assets.credit);
                });

                let Investments_Total = 0;
                balancesheetphase3portdata[1].forEach(investments => {
                    Investments_Total = Investments_Total + (Number(investments.opening) + investments.debit) - (investments.credit);
                });

                let Bank_Accounts_Total = 0;
                balancesheetphase3portdata[2].forEach(bank_accounts => {
                    Bank_Accounts_Total = Bank_Accounts_Total + (Number(bank_accounts.opening) + bank_accounts.debit) - (bank_accounts.credit);
                });

                let Cash_In_Hand_Total = 0;
                balancesheetphase3portdata[3].forEach(cash_in_hand => {
                    Cash_In_Hand_Total = Cash_In_Hand_Total + (Number(cash_in_hand.opening) + cash_in_hand.debit) - (cash_in_hand.credit);
                });

                let Cgst_Receivable_Total = 0;
                balancesheetphase3portdata[4].forEach(cgst_receivable => {
                    Cgst_Receivable_Total = Cgst_Receivable_Total + (Number(cgst_receivable.opening) + cgst_receivable.debit) - (cgst_receivable.credit);
                });

                let Cheque_In_Hand_Total = 0;
                balancesheetphase3portdata[5].forEach(cheque_in_hand => {
                    Cheque_In_Hand_Total = Cheque_In_Hand_Total + (Number(cheque_in_hand.opening) + cheque_in_hand.debit) - (cheque_in_hand.credit);
                });

                let Deposites_Total = 0;
                balancesheetphase3portdata[6].forEach(deposites => {
                    Deposites_Total = Deposites_Total + (Number(deposites.opening) + deposites.debit) - (deposites.credit);
                });

                let Igst_Receivable_Total = 0;
                balancesheetphase3portdata[7].forEach(igst_receivable => {
                    Igst_Receivable_Total = Igst_Receivable_Total + (Number(igst_receivable.opening) + igst_receivable.debit) - (igst_receivable.credit);
                });

                let Interim_Cgst_Receivable_Total = 0;
                balancesheetphase3portdata[8].forEach(interim_cgst_receivable => {
                    Interim_Cgst_Receivable_Total = Interim_Cgst_Receivable_Total + (Number(interim_cgst_receivable.opening) + interim_cgst_receivable.debit) - (interim_cgst_receivable.credit);
                });

                let Interim_Igst_Receivable_Total = 0;
                balancesheetphase3portdata[9].forEach(interim_igst_receivable => {
                    Interim_Igst_Receivable_Total = Interim_Igst_Receivable_Total + (Number(interim_igst_receivable.opening) + interim_igst_receivable.debit) - (interim_igst_receivable.credit);
                });

                let Interim_Sgst_Receivable_Total = 0;
                balancesheetphase3portdata[10].forEach(interim_sgst_receivable => {
                    Interim_Sgst_Receivable_Total = Interim_Sgst_Receivable_Total + (Number(interim_sgst_receivable.opening) + interim_sgst_receivable.debit) - (interim_sgst_receivable.credit);
                });

                let Loans_And_Advance_Total = 0;
                balancesheetphase3portdata[11].forEach(loans_and_advance => {
                    Loans_And_Advance_Total = Loans_And_Advance_Total + (Number(loans_and_advance.opening) + loans_and_advance.debit) - (loans_and_advance.credit);
                });

                let Sgst_Receivable_Total = 0;
                balancesheetphase3portdata[12].forEach(sgst_receivable => {
                    Sgst_Receivable_Total = Sgst_Receivable_Total + (Number(sgst_receivable.opening) + sgst_receivable.debit) - (sgst_receivable.credit);
                });

                let Stock_In_Hand_Total = 0;
                balancesheetphase3portdata[13].forEach(stock_in_hand => {
                    Stock_In_Hand_Total = Stock_In_Hand_Total + (Number(stock_in_hand.opening) + stock_in_hand.debit) - (stock_in_hand.credit);
                });

                let Sundry_Debtors_Total = 0;
                balancesheetphase3portdata[14].forEach(sundry_debtors => {
                    Sundry_Debtors_Total = Sundry_Debtors_Total + (Number(sundry_debtors.opening) + sundry_debtors.debit) - (sundry_debtors.credit);
                });

                let Misc_Expense_Total = 0;
                balancesheetphase3portdata[15].forEach(misc_expense => {
                    Misc_Expense_Total = Misc_Expense_Total + (Number(misc_expense.opening) + misc_expense.debit) - (misc_expense.credit);
                });              
